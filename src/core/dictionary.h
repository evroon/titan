...
class Dictionary {
   ...
   public:
    ...
    bool contains(const KEY &k) const {
        if (mymap.count(k) > 0) return true;
    ...
    VAL &get(const KEY &key) { return mymap[key]; }

    // Operators
    const VAL &operator[](const KEY &key) const { return mymap[key]; }
    VAL &operator[](const KEY &key) { return mymap[key]; }
};
