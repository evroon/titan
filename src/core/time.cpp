...
void Time::Update() { absolute_time = get_time_since_start(); }
...
void Time::OnUpdate() {
    ...
    if (!game_paused) game_time += delta_time;

    LastTime = absolute_time;

    for (int c = 0; c < uitimers.size(); c++) uitimers[c]->update();
}

void Time::restart() { Time::absolute_time = 0; }

int Time::get_deltatime() const { return delta_time; }

int Time::get_absolutetime() const { return absolute_time; }

int Time::get_gametime() const { return game_time; }
...
float Stopwatch::stop() {
    ...
    for (int c = 1; c < history.size(); c++) result_averaged += history[c];
...
bool RenderTarget::should_update() {
    if (fps_locked) {
        ...
        if (i.passedsec) fps_measured = i.FPS;

        if (i.needsupdate) delta_time = i.delta_time;
