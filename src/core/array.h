...
class Array {
   ...
   public:
    ...
    static Array<VAL> build_array(VAL* arr, int size) {
        Array<VAL> result;
        for (int c = 0; c < size; c++) result.push_back(arr[c]);
    ...
    void push_back(const Array<VAL>& a) {
        for (int c = 0; c < a.size(); c++) push_back(a[c]);
    }
    ...
    const VAL& operator[](const int i) const { return vec[i]; }
