...
public:
	...
	Rect2<T> crop(const T &all) const
	{
		return crop(all, all, all, all);
	}
	...
	Rect2<T> extend(const T &all) const
	{
		return extend(all, all, all, all);
	}
	...
	Array<Rect2<T>> horiz_split_3(const T &boundary_1, const T &boundary_2) const
	{
		...
		r.push_back(rect2(get_left() + boundary_1, get_left() + boundary_2, get_top(), get_bottom()));
