...
public:
	...
	mat4 inverse() const
	{
		...
		r[0] =  (m[5] * m[10] - m[9] * m[6]) * idet;
		r[1] = -(m[1] * m[10] - m[9] * m[2]) * idet;
		r[2] =  (m[1] * m[6]  - m[5] * m[2]) * idet;
		r[3] = 0.0;
		r[4] = -(m[4] * m[10] - m[8] * m[6]) * idet;
		r[5] =  (m[0] * m[10] - m[8] * m[2]) * idet;
		r[6] = -(m[0] * m[6]  - m[4] * m[2]) * idet;
		r[7] = 0.0;
		r[8] =  (m[4] * m[9] - m[8] * m[5]) * idet;
		r[9] = -(m[0] * m[9] - m[8] * m[1]) * idet;
		r[10] = (m[0] * m[5] - m[4] * m[1]) * idet;
		r[11] = 0.0;
