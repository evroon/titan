...
public:
	...
	Vec4(Vec2<T> xy, T p_z, T p_w) { x = xy.x; y = xy.y; z = p_z; w = p_w; }
	Vec4(T p_x, T p_y, Vec2<T> zw) { x = p_x; y = p_y; z = zw.x; w = zw.y; }
	Vec4(T p_x, Vec2<T> yz, T p_w) { x = p_x; y = yz.x; z = yz.y; w = p_w; }
