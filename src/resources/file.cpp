...
String File::get_absolute_path() const {
    if (!is_absolute_path()) return ASSETS_DIR + path;
...
String File::get_relative_path() const {
    if (path.starts_with(ASSETS_DIR)) {
        ...
        while (new_path.starts_with("/")) new_path = new_path.substr(1, -1);

        if (new_path.size() == 0) new_path = "/";
...
bool File::is_absolute_path() const { return path.starts_with(ASSETS_DIR); }
...
Array<File> File::listdir() const {
    ...
    while ((r = readdir(d)) != nullptr) {
        String s = r->d_name;
        if (s != ".." && s != ".") result.push_back(path + "/" + s);
    }
...
void File::correct_path() {
    if (!is_absolute_path()) path = ASSETS_DIR + path;
