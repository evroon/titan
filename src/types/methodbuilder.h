...
#define REG_CSTR_FULL(TYPE, TYPENAME, ARGC)                         \
    MethodBuilder::register_constructor(                            \
        ConstructorBuilder::build_##ARGC<TYPE>(), ParameterNames(), \
        VariantType(StringName(TYPENAME)))

#define REG_CSTR(ARGC)                                                       \
    REG_CSTR_FULL(CLASSNAME, CLASSNAME::get_type_name_static().get_source(), \
                  ARGC)
...
#define REG_CSTR_FULL_OVRLD_1(TYPE, TYPENAME, ARG_0)           \
    MethodBuilder::register_constructor(                       \
        ConstructorBuilder::build_1_overloaded<TYPE, ARG_0>(), \
        ParameterNames(), VariantType(StringName(TYPENAME)))

#define REG_CSTR_FULL_OVRLD_2(TYPE, TYPENAME, ARG_0, ARG_1)           \
    MethodBuilder::register_constructor(                              \
        ConstructorBuilder::build_2_overloaded<TYPE, ARG_0, ARG_1>(), \
        ParameterNames(), VariantType(StringName(TYPENAME)))

#define REG_CSTR_FULL_OVRLD_3(TYPE, TYPENAME, ARG_0, ARG_1, ARG_2)           \
    MethodBuilder::register_constructor(                                     \
        ConstructorBuilder::build_3_overloaded<TYPE, ARG_0, ARG_1, ARG_2>(), \
        ParameterNames(), VariantType(StringName(TYPENAME)))

#define REG_CSTR_FULL_OVRLD_4(TYPE, TYPENAME, ARG_0, ARG_1, ARG_2, ARG_3) \
    MethodBuilder::register_constructor(                                  \
        ConstructorBuilder::build_4_overloaded<TYPE, ARG_0, ARG_1, ARG_2, \
                                               ARG_3>(),                  \
        ParameterNames(), VariantType(StringName(TYPENAME)))

#define REG_CSTR_OVRLD_1(ARG_0) \
    REG_CSTR_FULL_OVRLD_1(      \
        CLASSNAME, CLASSNAME::get_type_name_static().get_source(), ARG_0)

#define REG_CSTR_OVRLD_2(ARG_0, ARG_1)                                    \
    REG_CSTR_FULL_OVRLD_2(CLASSNAME,                                      \
                          CLASSNAME::get_type_name_static().get_source(), \
                          ARG_0, ARG_1)

#define REG_CSTR_OVRLD_3(ARG_0, ARG_1, ARG_2)                             \
    REG_CSTR_FULL_OVRLD_3(CLASSNAME,                                      \
                          CLASSNAME::get_type_name_static().get_source(), \
                          ARG_0, ARG_1, ARG_2)

#define REG_CSTR_OVRLD_4(ARG_0, ARG_1, ARG_2, ARG_3)                      \
    REG_CSTR_FULL_OVRLD_4(CLASSNAME,                                      \
                          CLASSNAME::get_type_name_static().get_source(), \
...
#define REG_METHOD(METHOD)                                                     \
    REG_METHOD_FULL(CLASSNAME, CLASSNAME::get_type_name_static().get_source(), \
                    METHOD)
...
#define REG_PROPERTY_FULL(TYPE, TYPENAME, NAME)                         \
    MethodBuilder::register_property(                                   \
        ...
        REG_METHOD_FULL(TYPE, TYPENAME, set_##NAME), StringName(#NAME), \
        VariantType(StringName(TYPENAME)))
...
#define REG_PROPERTY(NAME)                                                 \
    MethodBuilder::register_property(                                      \
        ...
        VariantType(                                                       \
            StringName(CLASSNAME::get_type_name_static().get_source())))

#define REG_PROPERTY_NO(TYPE, NAME) REG_PROPERTY_FULL(TYPE, #TYPE, NAME)

// register static function
#define REG_STATIC_FUNC(NAME, METHOD)                             \
    MethodBuilder::reg_static_func(FunctorBuilder::build(METHOD), \
                                   StringName(NAME), ParameterNames())
...
struct FunctorBuilder {
    ...
    static std::function<VAR(VAR, VAR, VAR)> build(
        std::function<VAR(T, ARG_0, ARG_1)> f) {
    ...
    template <typename T, typename ARG_0>
    static std::function<void(VAR, VAR)> build(
        std::function<void(T, ARG_0)> f) {
        return [f](VAR v, VAR arg_0) { f(v, arg_0); };
    }

    template <typename T, typename ARG_0, typename ARG_1>
    static std::function<void(VAR, VAR, VAR)> build(
        std::function<void(T, ARG_0, ARG_1)> f) {
...
struct ConstructorBuilder {
    ...
    static std::function<Variant(Variant, Variant, Variant)> build_3() {
        return [](Variant arg_0, Variant arg_1, Variant arg_2) {
            return new T(arg_0, arg_1, arg_2);
        };
    }
    template <typename T>
    static std::function<Variant(Variant, Variant, Variant, Variant)>
    build_4() {
    ...
    static std::function<Variant(Variant, Variant, Variant)>
    build_3_overloaded() {
        return [](Variant arg_0, Variant arg_1, Variant arg_2) {
            return new T(cast<ARG_0>(arg_0), cast<ARG_1>(arg_1),
                         cast<ARG_2>(arg_2));
    ...
    build_4_overloaded() {
        return [](Variant arg_0, Variant arg_1, Variant arg_2, Variant arg_3) {
            return new T(cast<ARG_0>(arg_0), cast<ARG_1>(arg_1),
                         cast<ARG_2>(arg_2), cast<ARG_3>(arg_3));
        };
    }
};
