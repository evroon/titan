...
{
	return treeview->get_font()->get_height();
}
...
void TreeElement::search(const String & p_text)
{
	TreeElement* current = childs;

	while (current)
	{
		current->set_visible(current->get_text().starts_with(p_text));
...
TreeElement * TreeElement::get_next_visible()
{
	...
	if (current->expanded && current->childs)
	...
	case UIEvent::MOUSE_PRESS:
		...
		if (ui_event->press_type == UIEvent::DOWN)
		{
			...
			if (elm)
			{
				...
				if (collapse_icon.is_in_box(ui_event->pos))
...
void TreeView::position_items()
{
	if (roots.size() > 0)
	{
		...
		extra_space = area.get_bottom() - roots[0]->position(top);
