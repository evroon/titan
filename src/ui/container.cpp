...
void Container::set_glue_vert(bool p_glue_vert) { is_vert = p_glue_vert; }
...
void Container::position_childs() {
    if (!is_vert) {
        ...
        for (int c = 0; c < children.size(); c++) {
            ...
            control->set_anchors(Control::ANCHOR_BEGIN, Control::ANCHOR_BEGIN,
                                 Control::ANCHOR_BEGIN, Control::ANCHOR_END);
...
vec2 Container::get_required_size() const {
    ...
    for (int c = 0; c < children.size(); c++) {
        ...
        if (is_vert) {
            ...
            if (width < req_size.x) width = req_size.x;
        } else {
            width += req_size.x;

            if (height < req_size.y) height = req_size.y;
        }
    }
...
int Container::get_child_count() const { return children.size(); }
