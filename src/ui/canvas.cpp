...
CanvasLayer::CanvasLayer(int p_level) { level = p_level; }
...
void Canvas::remove_context_tip() {
    if (!context_tip) return;

    if (last_hover == context_tip) last_hover = NULL;
...
void Canvas::remove_context_menu() {
    ...
    if (context_menu->get_focused()) focused = NULL;
...
void Canvas::draw() {
    ...
    if (dialog) dialog->check_size_changed();
    ...
    if (dialog) dialog->draw();

    if (context_menu) context_menu->draw();
...
Color CanvasTheme::get_selection_color() const { return selection_color; }
