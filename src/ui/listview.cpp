...
int ListElement::get_index() { return index; }
            ...
            if (ui_event->press_type == UIEvent::DOWN) {
                ...
                if (index == -1)
                    ...
                    if (KEYBOARD->is_button_pressed(Key::KEY_LSHIFT)) {
                        if (selection_first == -1) selection_first = selected;

                        if (selection_first == -1) selection_first = 0;
                ...
                switch (ui_event->key.key_type) {
                    ...
                    case Key::KEY_UP:
                        if (ui_event->mod.type & ModKey::KEY_MOD_SHIFT) {
                            ...
                            if (!selecting) set_selection_begin(selected);
...
int ListView::get_position(const vec2& p_pos) {
    ...
    for (int c = 0; c < items.size(); c++) {
        if (items[c].is_in_area(p_pos)) return c;
    }
    return -1;
}
...
int ListView::get_selected() const { return selected; }

void ListView::move_selected_begin() { set_selected(0); }

void ListView::move_selected_end() { set_selected(items.size() - 1); }
...
void ListView::delete_selection() {
    if (selected != -1)
        ...
        for (int c = selection_begin; c <= selection_end; c++) remove_item(c);
    }
}
...
void ListView::stop_search() {
    for (int c = 0; c < items.size(); c++) items[c].set_visible(true);
}
...
void ListView::remove_item(int p_index) {
    ...
    if (p_index == selected) selected = -1;

    if (p_index == highlighted) highlighted = -1;
...
void ListView::push_back_item(const String& p_text, Texture2D* p_icon) {
    ...
    if (p_icon) item.set_icon(p_icon);
