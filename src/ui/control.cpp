...
void Control::set_anchor_top(int p_type) { anchors[3] = (AnchorType)p_type; }

void Control::set_anchor_bottom(int p_type) { anchors[1] = (AnchorType)p_type; }

void Control::set_anchor_left(int p_type) { anchors[0] = (AnchorType)p_type; }

void Control::set_anchor_right(int p_type) { anchors[2] = (AnchorType)p_type; }
...
int Control::get_anchor_top() const { return anchors[3]; }

int Control::get_anchor_bottom() const { return anchors[1]; }

int Control::get_anchor_left() const { return anchors[0]; }

int Control::get_anchor_right() const { return anchors[2]; }
...
vec2 Control::get_size() const { return area.size; }
...
int Control::get_level() const { return level; }
...
void Control::set_visible(bool p_visible) { visible = p_visible; }

bool Control::get_visible() const { return visible; }
...
void Control::size_changed() {
    ...
    if (pos[2] - pos[0] < minimum_size.x) pos[2] = pos[0] + minimum_size.x;

    if (pos[3] - pos[1] < minimum_size.y) pos[3] = pos[1] + minimum_size.y;

    rect2 new_area = rect2(pos[0], pos[2], pos[3], pos[1]);

    if (pos[2] > top_right.x) new_area.pos.x -= pos[2] - top_right.x;

    if (pos[3] > top_right.y) new_area.pos.y -= pos[3] - top_right.y;
    ...
    if (pos_changed || size_changed) update();

    if (pos_changed) notification(NOTIFICATION_TRANSLATED);
    ...
    for (int x = 0; x < 3; x++) {
        for (int y = 0; y < 3; y++) {
            ...
            vec4 bounds = vec4(vec2(b.x, b.y) / draw_command.tex->get_size().x,
                               vec2(b.z, b.w) / draw_command.tex->get_size().y);
