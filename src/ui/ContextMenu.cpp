	...
	case NOTIFICATION_DRAW:
		...
		for (int c = 0; c < items.size(); c++)
		{
			...
			if (items[c].icon)
			{
				...
				icon_area = rect2(item_area.get_left(), item_area.get_left() + tex_width, item_area.get_top(), item_area.get_bottom());
...
void ContextMenu::add_item(Texture2D* p_icon, const String& p_text, const Connection& p_connection)
{
...
String ContextMenu::get_text(int p_index) const
{
	return items[p_index].text;
}

int ContextMenu::get_selected() const
{
	return selected;
}
...
void ContextTip::close()
{
	get_canvas()->remove_context_tip();
}
