...
void TreeElement::set_index(int p_index) { index = p_index; }

int TreeElement::get_index() { return index; }
...
bool TreeElement::get_visible() const { return visible; }
...
bool TreeElement::search(const String& p_text) {
    ...
    while (current) {
        if (current->search(p_text)) v = true;

        current = current->next;
    }

    if (text.contains(p_text)) v = true;
...
TreeElement* TreeElement::get_item(const String& p_name) {
    ...
    while (current) {
        if (current->get_item(p_name)) return current;
...
TreeElement* TreeElement::get_next_visible() {
    ...
    if (current->expanded && current->childs && current->visible) {
        ...
        while (current->next && !current->visible) current = current->next;
...
TreeElement* TreeView::create_item(TreeElement* p_parent) {
    ...
    if (p_parent)
        ...
        if (roots.size() > 0) roots.getlast()->next = item;
