...
void Image::set_bounds_in_pixels(vec4 p_bounds) {
    bounds = vec4(vec2(p_bounds.x, p_bounds.y) / get_texture_size().x,
...
vec2 Image::get_required_size() const { return get_texture_size(); }

vec2 Image::get_texture_size() const { return texture->get_size(); }
