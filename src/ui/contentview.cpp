...
vec2 ContentView::get_required_size() const { return vec2(100); }

vec2 ContentView::get_full_size() const { return vec2(100); }
...
void ContentView::browse_to(const String &path) {
    ...
    for (int c = 0; c < names.size(); c++) add_item(names[c]);

    history.push(path);
}

void ContentView::clear_history() { history.clear(); }
...
void ContentView::go_home() { browse_to("assets/"); }
