...
void Vec3Field::value_input()
{
	...
	T_LOG(vec3(v_x, v_y, v_z).to_string());

	set_value(vec3(v_x, v_y, v_z));
}
...
vec2 Vec3Field::get_required_size() const
{
	...
	return vec2(
		size_x.x + size_y.x + size_z.x + 
		margin[0] + margin[1] * 2.0f + margin[2] * 2.0f + margin[3],
		size_x.y);
}
...
TransformField::TransformField(const Transform& p_variant) : PropertyControl(p_variant)
{
	init();
}

TransformField::TransformField(const Variable & p_variable) : PropertyControl(p_variable)
{
	init();
}
...
void TransformField::init()
{
	...
	pos.connect("value_changed", this, "value_changed");
	size.connect("value_changed", this, "value_changed");
	rotation.connect("value_changed", this, "value_changed");
}
...
void TransformField::value_input()
{
	...
	set_value(Transform(v_pos, v_size));
}
...
vec2 TransformField::get_required_size() const
{
	...
	return vec2(max_x, size_pos.y + size_size.y + size_rotation.y + margin[0] + margin[1] + margin[2] + margin[3]);
}
...
void TransformField::position_items()
{
	...
	pos.set_margins(4, size_pos.y + margin[0], 4, margin[0]);
	size.set_margins(4, size_size.y + margin[1] + size_pos.y + margin[0], 4, margin[1] + size_pos.y + margin[0]);
	rotation.set_margins(4, size_rotation.y + margin[2] + size_size.y + margin[1] + size_pos.y + margin[0], 4, margin[2] + size_size.y + margin[1] + size_pos.y + margin[0]);
}
